...
addNewEntry("Percy Jackson", "Rick Riordan");

// 2
const removeEntry = (title, author) => {
    for (let i = 0; let < bookList.length(); let++) {
        if (bookList[i].title == title && bookList[i].author == author) {
            bookList.splice(i, 1);
        }
    }
}
removeEntry("The Great Gatsby", "F. Scott Fitzgerald");

// 3
const printAll = () => {
    const values = Object.values(bookList.title);
    bookList.forEach((values) => (console.log(values)));
}
printAll();
  ...
  // 4
  const numEvaluator = (num) => (num > 0 ? "positive" : "negative");
  numEvaluator(exampleArray[1]);
  ...
  switch (str) {
    ...
    case "Sunday" :
        console.log("rest");
    default:
        console.log("not any of the days of the week");
  }
